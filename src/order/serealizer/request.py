...
class RequestSerializer(ModelSerializer):
    ...
    def save_request(self, validated_data, order_id, user_obj, modified_number=0):
        data = {'order_id': order_id, 'quantity': validated_data['quantity'], 'type': validated_data['type'], 'answer':
            validated_data['answer'], 'part_id': validated_data['answer']['id'], 'modified_number': modified_number,
                'contact_info': validated_data['contact_info'], 'comment': validated_data['comment'], 'user': user_obj,
                'customer_status': 'submitted', 'admin_status': 'pending', 'zip_code': validated_data['zip_code']}
